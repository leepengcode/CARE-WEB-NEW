...
export default function MostLikeProperty() {
    ...
    const fetchProperties = async () => {
      ...
      try {
        ...
        filteredProperties.sort(
          (a, b) => (b.favourite_count || 0) - (a.favourite_count || 0)
        );
  ...
  const handlePropertyClick = (property) => {
    sessionStorage.setItem("mostLikedScroll", window.scrollY);
  ...
  return (
    <PageComponents>
      <div className="w-full max-w-7xl mx-auto py-4 md:py-5 lg:px-10">
        <div className="flex justify-between items-center mb-3">
          <h2
            className={`text-md md:text-2xl font-bold bg-gradient-to-r from-pink-600 to-purple-600 bg-clip-text text-transparent transition-all duration-500 ease-in-out transform ${
        ...
        {Properties.length === 0 && !loading && !error && (
          ...
          >
            ...
            <h3 className="text-xl font-semibold text-gray-600 mb-2">
              No Most Liked Properties Yet
            </h3>
            <p className="text-gray-500">
              Properties will appear here once users start liking them.
            </p>
          </div>
        )}
      </div>
