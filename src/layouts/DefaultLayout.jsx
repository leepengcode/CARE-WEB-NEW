...
export default function DefaultLayout() {
  ...
  return (
    <div className="min-h-full bg-gray-50">
          ...
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center">
              <div className="shrink-0">
                <img className="h-20 w-auto" src="https://externalchecking.com/logo/care.png" alt="Company Logo" />
              </div>
            </div>
                    ...
                    <>
                      <button
                        ...
                        className={classNames(
                          ...
                          openMenu === idx ? 'text-blue-600' : 'text-gray-700 hover:text-blue-600'
                        )}
                      >
                        {item.name}
                        <ChevronDownIcon
                          className={classNames('w-4 h-4 transition-transform', openMenu === idx ? 'rotate-180' : '')}
                        />
                      </button>
                      {/* Dropdown */}
                      <div
                        className={classNames(
                          ...
                          openMenu === idx ? 'max-h-96 opacity-100 visible' : 'max-h-0 opacity-0 invisible'
                        )}
            ...
            <div className="hidden md:flex items-center gap-4">
              {userToken ? (
                ...
                <Menu as="div" className="relative">
                  ...
                  <Menu.Items className="absolute right-0 mt-2 w-24 rounded-md shadow-lg bg-white py-1">
                    <Menu.Item>
                      {({ active }) => (
                        <Link
                          to="/login"
            ...
            <div className="md:hidden flex items-center">
              <button onClick={() => setMobileMenuOpen(true)} className="p-2 text-gray-700 hover:text-blue-600">
      ...
      {/* Overlay */}
      {mobileMenuOpen && (
        <div onClick={() => setMobileMenuOpen(false)} className="fixed inset-0 bg-black bg-opacity-30 z-40" />
      )}
      ...
      >
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <button onClick={() => setMobileMenuOpen(false)} className="p-2 bg-blue-600 text-white rounded-full">
              ...
              {item.children ? (
                <>
                  ...
                  >
                    {item.name}
                    <ChevronDownIcon
                      className={`h-5 w-5 transition-transform ${openMobileMenu === idx ? 'rotate-180' : ''}`}
                    />
                  </button>
                  <div
                    className={classNames(
                      'overflow-hidden transition-all duration-200',
                      openMobileMenu === idx ? 'max-h-96 opacity-100 visible' : 'max-h-0 opacity-0 invisible'
                    )}
                    style={{ transitionProperty: 'max-height, opacity' }}
                  >
                    <div className="mt-2 pl-4 space-y-2">
                      {item.children.map((subItem) => (
                        <NavLink
                          ...
                          className={({ isActive }) =>
                            classNames(
                              isActive ? 'bg-red-600 text-white font-semibold' : 'text-gray-600 hover:text-blue-500',
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    classNames(
                      isActive ? 'bg-blue-600 text-white' : 'text-gray-700 hover:text-blue-500',
